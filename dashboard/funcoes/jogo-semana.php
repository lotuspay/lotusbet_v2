...
//$gameCodes = ['vs5luckytig', '126', 'aviator', 'mines', '98', 'vs10bbbonanza'];
//$placeholders = implode(',', array_fill(0, count($gameCodes), '?'));

$stmt = $pdo->prepare("SELECT id, game_name, game_img, game_code FROM bet_jogos WHERE game_ativado = 1 AND game_destacado = 1 ORDER BY RAND() LIMIT 6");
$stmt->execute();
