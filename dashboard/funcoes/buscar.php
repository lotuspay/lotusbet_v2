...
if ($q !== '' && preg_match('/^[\p{L}\p{N}\s]{1,100}$/u', $q)) {
    $stmt = $pdo->prepare("SELECT id, game_name, game_img, game_code FROM bet_jogos WHERE game_name LIKE ? AND game_ativado = 1 LIMIT 10");
    $stmt->execute(["%$q%"]);
    ...
    <table class="busca-tabela-resultado">
        <tr>
            ...
            <td class="col-jogar">
                <button class="btn-jogar" data-modal="modal-slots'.$row['id'].'" data-url="" data-game-id="'.$row['game_code'].'">Jogar</button>
            </td>
        </tr>
    </table>';
        }
